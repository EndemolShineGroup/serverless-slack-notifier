...
- **Node 8 LTS:** `serverless-slack-notifier` has a minimum Node version
...
``` bash
git clone https://github.com/EndemolShineGroup/serverless-slack-notifier.git
cd serverless-slack-notifier
yarn
yarn test
```
...
We want to ensure that `serverless-slack-notifier` is secure for everyone. If
